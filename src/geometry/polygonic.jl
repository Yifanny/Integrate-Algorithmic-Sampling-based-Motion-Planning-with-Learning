...
function show(this::T, color=:red) where {T<:Polygonic}
    real_v = [[this.V[1][1]+1, this.V[1][2]+2],
              [this.V[2][1]-1, this.V[2][2]+2],
              [this.V[3][1]-1, this.V[3][2]-2],
              [this.V[4][1]+1, this.V[4][2]-2]]
    for n = 1:this.N
        if n<this.N
            x = [this.V[n][1], this.V[n+1][1]]
            y = [this.V[n][2], this.V[n+1][2]]
            real_x = [real_v[n][1], real_v[n+1][1]]
            real_y = [real_v[n][2], real_v[n+1][2]]
        else
            x = [this.V[n][1], this.V[1][1]]
            y = [this.V[n][2], this.V[1][2]]
            real_x = [real_v[n][1], real_v[1][1]]
            real_y = [real_v[n][2], real_v[1][2]]
        end
        plot(x, y, "r-", linestyle="--")
        plot(real_x, real_y, "b-")
    end
end
...
function test()
    ...
    P = Polygon(V)
    """
    P = Rectangle([0.0, 0.0], 5.0, 5.0)
    clf()
    show(P)
    savefig("test_show_func.png")
