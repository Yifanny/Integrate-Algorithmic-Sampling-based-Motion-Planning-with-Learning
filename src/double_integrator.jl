...
function test_connection()
    init = [8.305,1784.037]
    init_v = 24.98
    next_p_after_init = [8.317,1786.539]
    goal = [12.758,1833.881]
    goal_v = 26.89
    next_p_after_goal = [13.066,1836.543]
    init_angel = atan(next_p_after_init[2] - init[2], next_p_after_init[1] - init[1])
    goal_angel = atan(next_p_after_goal[2] - goal[2], next_p_after_goal[1] - goal[1])
    s0 = Vec4f(init[1], init[2], init_v*cos(init_angel), init_v*sin(init_angel))
    s1 = Vec4f(goal[1], goal[2], goal_v*cos(goal_angel), goal_v*sin(goal_angel))
    println(s0)
    println(s1)
