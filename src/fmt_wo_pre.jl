...
mutable struct FMTree
    ...
    function FMTree(s_init::Vec4f, s_goal::Vec4f, N, world, right_prob::Float64, left_prob::Float64, keep_prob::Float64, lane_id::Int64, frames, frame_id)
        ...
        points = read_sample_points("wo_pre_sample_points/9_f_id_"*string(frame_id)*".txt")
        ...
        N_rest = N - length(Pset) - 1
        N_cprm = length(Pset)
        println("N_rest:", N_rest)
        while length(Pset) < N - 1
            for i=2:N_cprm
                x = gaussianrn(Pset[i][1], 1)
                y = gaussianrn(Pset[i][2], 1)
                vx = gaussianrn(Pset[i][3], 1)
                vy = gaussianrn(Pset[i][3], 1)
                p = Vec4f(x, y, vx, vy)
                if isValid(world, p)
                    push!(Pset, p)
                end
