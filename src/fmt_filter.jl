...
mutable struct FMTree
    ...
    function FMTree(s_init::Vec4f, s_goal::Vec4f, N, world)
        ...
        uniform_percent = 1.0
        N = N - 2
        N_uniform = Int(round(N * uniform_percent))
        N_prob = N - N_uniform
        N = N + 2
        for n = 1:N_uniform
            x = myrn(world.x_min[1], world.x_max[1])
            y = myrn(world.x_min[2], world.x_max[2])
            x_v = myrn(world.v_min[1], world.v_max[1])
            y_v = myrn(world.v_min[1], world.v_max[1])
            rand_num = rand()
            if 0 < x < 0.65 || rand_num >= 0.9
                p = Vec4f(x, y, x_v, y_v)
                if isValid(world, p)
                    push!(Pset, p)
                end
